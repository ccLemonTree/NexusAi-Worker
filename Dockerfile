...
# Base: python:3.12-slim  (no GPU needed; inference runs on remote Triton server)
# Entry: python kafka_main.py
# ─────────────────────────────────────────────────────────────────────────────
FROM python:3.12-slim

# Disable .pyc files and ensure logs stream immediately to stdout
ENV PYTHONUNBUFFERED=1 \
    PYTHONDONTWRITEBYTECODE=1 \
    # Default project root inside the container
    NEXUSAI_HOME=/app
...
# MILVUS_CLIENT            Milvus connection string
