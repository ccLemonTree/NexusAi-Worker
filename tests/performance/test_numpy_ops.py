...
def numpy_heavy_ops():
    ...
    for i in range(len(boxes1)):
        ...
        boxe.append(int(np.argmax(boxes1[i, 5:])))  # 这里！
        zzz.append(np.array(boxe))
    boxes = np.array(zzz)

    # argsort
    confs = boxes[:, 4]
    boxes = boxes[np.argsort(-confs)]  # 这里！
    ...
    while boxes.shape[0] > 0 and len(keep_boxes) < 100:
        ...
        box2 = boxes[:, :4]
        iou = np.random.rand(boxes.shape[0])  # 简化
...
def run_test(concurrent):
    print(f"\n{'='*80}")
    print(f"测试 numpy 操作 (argmax + argsort + NMS 循环)")
